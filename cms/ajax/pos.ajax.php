...
class PosController{
	...
	public function loadProducts(){
		...
		if($this->category == "all"){
			...
			if($this->search == ""){
				...
				$url = "relations?rel=products,categories&type=product,category&linkTo=status_product&equalTo=1&orderBy=id_product&orderMode=DESC&startAt=".$this->startAt."&endAt=".$this->limit;
				$method = "GET";
				$fields = array();
	...
	public function newOrder(){
		...
		$url = "cashs?linkTo=date_created_cash,status_cash&equalTo=".date("Y-m-d").",1&select=status_cash";
		$method = "GET";
		$fields = array();
		...
		if($cash->status == 404){
			...
			$url = "cashs?linkTo=date_created_cash,status_cash&equalTo=".$yesterday.",1"; 
			$method = "GET";
			$fields = array();
